...
#             request.token = token  # Store token for later use
...
#     """
#     Build context using semantic search on past messages.
...
#     try:
#         # Fetch past messages from the session
...
# #         use_context = data.get("useContext", True)  # Allow disabling context per request
...
# #         if use_context and ENABLE_SEMANTIC_SEARCH:
...
# #         # Generate bot response from AI model with context
# #         bot_reply = generate_bot_response(enhanced_message, model)

# #         # Send to Node.js server to save in MongoDB (save ORIGINAL message, not enhanced)
# #         auth_header = request.headers.get("Authorization")
...
# #                     "userMessage": message,  # Save original message
...
# #             "botReply": bot_reply,
# #             "contextUsed": enhanced_message != message  # Inform client if context was used
# #         }), 200
...
#         is_edit = data.get("isEdit", False)  # Check if this is an edit
...
#         # Build semantic context if enabled and not an edit
#         enhanced_message = message
...
#             if enhanced_message != message:
#                 print(f"   âœ¨ Enhanced with context ({len(enhanced_message)} chars)")
...
#                     "botMessage": bot_reply,
#                     "model": model,
#                     "isEdit": is_edit  # Pass the isEdit flag
#                 },
#                 headers={
#                     "Authorization": auth_header,
#                     "Content-Type": "application/json"
...
#             if not node_response.ok:
#                 print(f"âŒ Node.js server error: {node_response.status_code} - {node_response.text}")
#                 raise Exception(f"Failed to save to MongoDB: {node_response.status_code}")

#             saved_data = node_response.json()
#             print(f"âœ… Successfully saved conversation to MongoDB for session {saved_data.get('session', {}).get('_id', 'unknown')}")

#         except requests.exceptions.RequestException as e:
#             print(f"âŒ Request error to Node.js server: {e}")
#             raise Exception(f"Failed to connect to Node.js server: {str(e)}")
#         except Exception as e:
...
# # ---------------- Semantic Search Config Endpoint ----------------
# @app.route("/api/semantic-config", methods=["GET"])
# @authenticate_token
# def get_semantic_config():
#     return jsonify({
#         "enabled": ENABLE_SEMANTIC_SEARCH and semantic_engine is not None,
#         "topN": TOP_N_RELEVANT_MESSAGES,
